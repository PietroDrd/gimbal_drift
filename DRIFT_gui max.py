...
class MotorControllerApp:
    def __init__(self, master):
        ...
        # Slider controls for motors with range 1000 to 2000
        self.sliders = []
        for i in range(4):
            slider = tk.Scale(master, from_=1000, to=2000, orient='horizontal', label=f'Motor {i+1}')
            slider.pack()
            slider.bind("<ButtonRelease-1>", self.update_motors)  # Update motors only on release
            self.sliders.append(slider)

        self.master_slider = tk.Scale(master, from_=1000, to=2000, orient='horizontal', label='Master Control')
